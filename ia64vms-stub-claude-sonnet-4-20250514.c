...
{
    const unsigned int LOW_BYTE_MASK = 0xFF;
    const unsigned int HIGH_BYTE_SHIFT = 8;
    
    return ((v & LOW_BYTE_MASK) << HIGH_BYTE_SHIFT) | 
           ((v >> HIGH_BYTE_SHIFT) & LOW_BYTE_MASK);
}

/* Initialize the socket connection, and wait for a client.  */

static unsigned int assign_channels(unsigned short *listen_channel)
{
...
static unsigned int set_socket_reuse(unsigned short listen_channel)
{
    struct _iosb iosb;
    int optval = 1;
...
static unsigned int bind_socket(unsigned short listen_channel, struct sockaddr_in *serv_addr)
{
    struct _iosb iosb;
    ILE2 serv_itemlst;
    unsigned int status;
...
static unsigned int set_socket_listen(unsigned short listen_channel)
{
    struct _iosb iosb;
    unsigned int status;
...
static unsigned int accept_connection(unsigned short listen_channel)
{
    struct _iosb iosb;
    unsigned int status;
...
static unsigned int get_client_info(struct sockaddr_in *cli_addr)
{
    struct _iosb iosb;
...
static void handle_error(const char *message, unsigned int status)
{
    term_puts(message);
    LIB$SIGNAL(status);
}
...
sock_init(void)
{
    ...
    status = assign_channels(&listen_channel);
    if (!(status & STS$M_SUCCESS))
    {
        handle_error("Failed to assign I/O channel(s)\n", status);
        return;
    }
    ...
    status = set_socket_reuse(listen_channel);
    if (!(status & STS$M_SUCCESS))
    {
    ...
    status = bind_socket(listen_channel, &serv_addr);
    if (!(status & STS$M_SUCCESS))
    {
    ...
    status = set_socket_listen(listen_channel);
    if (!(status & STS$M_SUCCESS))
    {
        handle_error("Failed to set socket passive\n", status);
        return;
    }
    ...
    status = accept_connection(listen_channel);
    if (!(status & STS$M_SUCCESS))
    {
    ...
    status = get_client_info(&cli_addr);
    if (!(status & STS$M_SUCCESS))
    {
        handle_error("Failed to get client name\n", status);
        return;
    }
...
get_teb(void)
{
    void *reg_value = (void *)__getReg(_IA64_REG_TP);
    return (pthread_t)reg_value;
}
...
{
    if (context == NULL || thread_info == NULL) {
        return EINVAL;
    }
...
malloc_callback(pthreadDebugClient_t caller_context, size_t size)
{
    ...
    const size_t MIN_ALLOCATION_SIZE = 1;
    
    if (size == 0 || size > SIZE_MAX - HEADER_SIZE) {
        return NULL;
    }
    
    int total_len = (int)(size + HEADER_SIZE);
    if (total_len < 0) {
        return NULL;
    }
    
    unsigned int result_addr = 0;
    unsigned int status = lib$get_vm(&total_len, &result_addr, 0);
    ...
    if (trace_pthreaddbg) {
        TERM_FAO("malloc_callback (!UL) -> !XA!/", size, result_addr);
    }
    
    *(unsigned int *)result_addr = (unsigned int)total_len;
    return (pthreadDebugAddr_t)((char *)result_addr + HEADER_SIZE);
}
...
pthread_debug_thd_seq_init(pthreadDebugId_t *id)
{
    ...
    int (*func_ptr)(void *, pthreadDebugId_t *) = 
        (int (*)(void *, pthreadDebugId_t *))pthread_debug_entries[1].func;
...
pthread_debug_thd_seq_destroy(void)
{
    ...
    int (*destroy_func)(void*) = (int (*)(void*))pthread_debug_entries[3].func;
...
pthread_debug_thd_get_info_addr(pthread_t thr, pthreadDebugThreadInfo_t *info)
{
    ...
    if (!pthread_debug_entries || !pthread_debug_entries[5].func) {
        return -1;
    }
    
    int (*func_ptr)(void *, pthread_t, pthreadDebugThreadInfo_t *) = 
        (int (*)(void *, pthread_t, pthreadDebugThreadInfo_t *))pthread_debug_entries[5].func;
    
    return func_ptr(debug_context, thr, info);
}
			   ...
			   pthreadDebugRegs_t *regs)
{
  ...
  typedef int (*get_reg_func_t)(void *, pthreadDebugId_t, pthreadDebugRegs_t *);
  get_reg_func_t func = (get_reg_func_t)pthread_debug_entries[6].func;
...
stub_pthread_debug_cmd(const char *cmd)
{
    ...
    int (*debug_func)(void *, const char *) = 
        (int (*)(void *, const char *))pthread_debug_entries[7].func;
...
threads_init(void)
{
    ...
    status = lib$find_image_symbol(&pthread_rtl_desc, &dbgext_desc, &dbgext_func);
    if (!(status & STS$M_SUCCESS)) {
        LIB$SIGNAL(status);
    }
    
    status = lib$find_image_symbol(&pthread_rtl_desc, &dbgsymtable_desc, &dbg_symtable);
    if (!(status & STS$M_SUCCESS)) {
        LIB$SIGNAL(status);
    }
    ...
    for (size_t i = 0; i < entry_count; i++) {
        ...
        status = lib$find_image_symbol(&pthread_debug_desc, &sym, &pthread_debug_entries[i].func);
        if (!(status & STS$M_SUCCESS)) {
            lib$signal(status);
        }
    }
    ...
    if (pthread_debug_entries[0].func != NULL) {
        status = ((int (*)())pthread_debug_entries[0].func)(&caller_context, &pthread_debug_callbacks, dbg_symtable, &debug_context);
        if (status != 0) {
...
str2pkt (const char *str)
{
  ...
  while (*str && gdb_blen < sizeof(gdb_buf) - 1) {
...
handle_q_packet (const unsigned char *pkt, unsigned int pktlen)
{
  ...
  const unsigned int XFER_UIB_LEN = sizeof(xfer_uib) - 1;
  const unsigned int QFTHREADINFO_LEN = sizeof(qfthreadinfo) - 1;
  const unsigned int QSTHREADINFO_LEN = sizeof(qsthreadinfo) - 1;
  const unsigned int QTHREADEXTRAINFO_LEN = sizeof(qthreadextrainfo) - 1;
  const unsigned int QSUPPORTED_LEN = sizeof(qsupported) - 1;
...
static void
handle_current_thread_query(void)
{
  gdb_buf[0] = '$';
  gdb_buf[1] = 'Q';
  gdb_buf[2] = 'C';
...
static void
handle_unwind_info_query(const unsigned char *pkt, unsigned int pktlen, unsigned int prefix_len)
{
  unsigned __int64 pc;
...
static void
handle_first_thread_query(unsigned int *first_thread, unsigned int *last_thread)
{
  gdb_buf[0] = '$';
  gdb_buf[1] = 'm';
  gdb_blen = 2;
...
static void
handle_subsequent_threads_query(unsigned int *first_thread, unsigned int *last_thread)
{
  gdb_buf[0] = '$';
  gdb_buf[1] = 'm';
  gdb_blen = 2;
...
static void
handle_thread_extra_info_query(const unsigned char *pkt, unsigned int prefix_len)
{
  pthread_t thr;
  unsigned int pos = prefix_len;
  pthreadDebugThreadInfo_t info;
...
static void
handle_supported_features_query(void)
{
  gdb_buf[0] = '$';
  gdb_blen = 1;
...
handle_v_packet (const unsigned char *pkt, unsigned int pktlen)
{
  ...
  static const unsigned int VCONTQ_LEN = sizeof(vcontq) - 1;
...
handle_packet (unsigned char *pkt, unsigned int len)
{
  ...
  switch (pkt[0])
    ...
    case 'g':
      ...
      break;
    case 'H':
      return handle_h_packet(pkt, len, pos);
    case 'k':
      SYS$EXIT (SS$_NORMAL);
      break;
    case 'm':
      return handle_memory_read(pkt, len, pos);
    case 'M':
      return handle_memory_write(pkt, len, pos);
    case 'p':
      return handle_register_read(pkt, len, pos);
    case 'q':
    ...
    case 's':
      ...
      break;
    case 'T':
      return handle_thread_status(pkt, len, pos);
    case 'v':
...
static int
handle_h_packet(unsigned char *pkt, unsigned int len, unsigned int pos)
{
  if (pkt[1] == 'g')
    {
      int res;
...
static int
handle_memory_read(unsigned char *pkt, unsigned int len, unsigned int pos)
{
...
static int
handle_memory_write(unsigned char *pkt, unsigned int len, unsigned int pos)
{
  unsigned __int64 addr;
  unsigned __int64 paddr;
  unsigned int l;
...
static int
handle_register_read(unsigned char *pkt, unsigned int len, unsigned int pos)
{
  unsigned int num = 0;
  struct ia64_all_regs *regs = get_selected_regs ();
...
static int
handle_thread_status(unsigned char *pkt, unsigned int len, unsigned int pos)
{
  if (!has_threads)
    {
      packet_ok ();
...
static int
handle_vms_extension(unsigned char *pkt, unsigned int len)
{
...
sock_write (const unsigned char *buf, int len)
{
  ...
  if (buf == NULL || len <= 0)
    {
      term_puts ("Invalid parameters for sock_write\n");
      return;
    }
...
static int find_dollar_sign(const unsigned char *buf, unsigned int len) {
    for (unsigned int i = 0; i < len; i++) {
        if (buf[i] == '$') {
            return i;
        }
...
static int find_sharp_sign(const unsigned char *buf, unsigned int start, unsigned int len) {
...
static int read_packet_data(unsigned int off) {
    struct _iosb iosb;
    unsigned int status;
    
    if (off >= sizeof(gdb_buf)) {
        return -1;
    }
...
static int validate_packet_checksum(unsigned int dollar_off, unsigned int sharp_off) {
    unsigned char chksum = 0;
    
    for (unsigned int i = dollar_off + 1; i < sharp_off; i++) {
        chksum += gdb_buf[i];
    }
    
    int received_checksum = hex2byte(gdb_buf + sharp_off + 1);
    return (received_checksum == chksum) ? 1 : 0;
}
...
static int one_command(void) {
    ...
    while (1) {
        ...
        while (1) {
            if (read_packet_data(off) < 0) {
                return -1;
            }
            
            if (off == 0) {
                dollar_off = find_dollar_sign(gdb_buf, gdb_blen);
                if (dollar_off < 0) {
                    off = 0;
                    continue;
                }
                sharp_off = find_sharp_sign(gdb_buf, dollar_off + 1, gdb_blen);
            } else if (sharp_off >= 0 && sharp_off < gdb_blen) {
                sharp_off = find_sharp_sign(gdb_buf, sharp_off, gdb_blen);
            }
            
            if (sharp_off >= 0 && sharp_off + 2 <= gdb_blen) {
                break;
            }
        ...
        if (validate_packet_checksum(dollar_off, sharp_off)) {
    ...
    unsigned int cmd_off = dollar_off + 1;
    unsigned int cmd_len = sharp_off - dollar_off - 1;
...
{
  if (mech == NULL || mech->chf$q_mch_esf_addr == 0) {
    return;
  }
...
do_debug (struct chf$mech_array *mech)
{
  ...
  if (scheduling_disabled && has_threads) {
...
excp_handler(struct chf$signal_array *sig, struct chf$mech_array *mech)
{
    ...
    if (handle_entry_breakpoint(code, pc)) {
        /* Entry breakpoint handled */
    }
    
    unsigned int ret = process_exception(code, sig64, mech, cnt);
...
static int handle_entry_breakpoint(unsigned int code, unsigned __int64 pc)
{
...
static unsigned int process_exception(unsigned int code, 
                                     struct chf64$signal_array *sig64,
                                     ...
                                     unsigned int cnt)
{
    switch (code) {
        ...
        case SS$_DEBUG & STS$M_COND_ID:
            if (trace_excp > 1) {
                display_detailed_exception(sig64, mech, cnt);
            }
            do_debug(mech);
...
static void display_detailed_exception(struct chf64$signal_array *sig64,
                                      struct chf$mech_array *mech,
                                      unsigned int cnt)
{
    struct _intstk *intstk = (struct _intstk *)mech->chf$q_mch_esf_addr;
...
trace_init (void)
{
  ...
  for (i = 0; i <= len; i++)
    {
      int is_delimiter = (i == len || resstring[i] == ',' || resstring[i] == ';');
      int has_content = (i != start);
      
      if (is_delimiter && has_content)
	{
	  int j;
	    ...
	    unsigned int linkflag, unsigned int cliflag)
{
  ...
  if (setup_entry_breakpoint() != SS$_NORMAL && entry_pc == 0) {
    while (one_command () == 0) {
      ;
    }
  }
...
static void process_image_trace(IMCB *imcb)
{
  unsigned int j;
...
static void print_segment_info(LDRISD *ldrisd)
{
...
static unsigned int setup_entry_breakpoint(void)
{
